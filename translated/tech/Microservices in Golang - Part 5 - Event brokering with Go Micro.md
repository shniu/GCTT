...
在本系列的前一部分中，我们谈到了用户认证和 JWT。在这一集中，我们将快速浏览 go-micro 的代理功能。
...
就是这样！这是一个微不足道的例子，因为我们的电子邮件服务隐式地收听单个 `user.created` 事件，但希望你能看到这种方法如何让你编写解耦的服务。
...
内置 go-micro 是 pubsub 层，位于代理层之上，但不需要第三方代理（如 NATS）。 但是这个功能真棒的部分在于它利用了 protobuf 的定义。 所以我们回到了低延迟二进制流的领域。 因此，让我们更新我们的用户服务，用 go-micro 的 pubsub 替换现有的 NATS 代理：
...
本文中的任何错误，错误或反馈，或任何您会发现有用的东西，请给我发一封[电子邮件](ewan.valentine89@gmail.com)。
